...
type Props = {
    ...
    {tags.slice(0,10).map((it, i) => (
      
      <Link key={i} href={`/posts/tagged/${it.slug}`}>
      <a style={{width: '32.5%'}} className="inline-block h-1/3">
<li className="flex justify-center rounded-lg mt-3 transition dark:border-none border border-grayish dark:bg-evening hover:bg-white bg-daylight dark:hover:bg-lightnight text-midnightish dark:text-white inline-block h-1/3 p-6 pt-4"
>{it.name}</li>
</a>
</Link>
))}
